...
import com.katappult.cloud.platform.generated.rest.dto.ReplayToCategoryLinkDTO;
...
@RestController
@RequestMapping("/katappult/cloud/app/v1/secured/api/replay/manytomany/Category")
public class ReplayToCategoryServiceFacade {
    ...
    public Serializable createLink(@RequestParam("roleAId") RestObjectFullId roleAId,
        ...
        service.createLink((Replay) roleAId.getPersistable(), (Category) roleBId.getPersistable(), container);
        return result;
    }
    ...
    public Serializable deleteLink(@RequestParam("roleAId") RestObjectFullId roleAId,
        ...
        Replay roleA = (Replay) roleAId.getPersistable();
    ...
    public Serializable moveFromRoleA(@RequestParam("roleAId") RestObjectFullId roleAId,
        ...
        Replay roleA = (Replay) roleAId.getPersistable();
        Replay targetRoleA = (Replay) targetRoleAId.getPersistable();
        Category roleB = (Category) roleBId.getPersistable();
    ...
    public Serializable getLink(@RequestParam("roleAId") RestObjectFullId roleAId,
        ...
        Replay roleA = (Replay) roleAId.getPersistable();
        Category roleB = (Category) roleBId.getPersistable();
        Optional<ReplayToCategoryLink> linkAsOptional = service.getLink(roleA, roleB, container);
    ...
    public Serializable getAllReplayInverseRoleBsManyToManyLinkToOfCategory(
                                     @RequestParam("roleBId") RestObjectFullId roleBId,
                                     @RequestParam("containerId") Container container){

        Category roleB = (Category) roleBId.getPersistable();
        List<Replay> roleAs = service.getAllReplayInverseRoleBsManyToManyLinkToOfCategory(roleB, container);
    ...
    public Serializable getAllRoleBsWithLink(@RequestParam("roleAId") RestObjectFullId roleAId,
        ...
        Replay roleA = (Replay) roleAId.getPersistable();
        Map<Category, ReplayToCategoryLink> links = service.getAllRoleBsWithLink(roleA, container);
        List dtos = links.keySet().stream().map(key -> ReplayToCategoryLinkDTO.from(key, links.get(key))).collect(Collectors.toList());
        result.setData(dtos);
        return result;
    }
