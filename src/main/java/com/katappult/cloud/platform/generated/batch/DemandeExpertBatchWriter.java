...
@Component("DemandeExpertBatchWriter")
@Scope("step")
public class DemandeExpertBatchWriter implements ItemWriter<AbstractLineBean> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DemandeExpertBatchWriter.class);

    @Autowired
    private KatappultCoreServicesHelper services;

    @Autowired
    private IDemandeExpertService service;
    ...
    public void write(final List<? extends AbstractLineBean> items) throws Exception {
        ...
        while(entityIterator.hasNext()) {
          ...
          if(command.startsWith("ADD_")){
              ...
              DemandeExpert entity = (DemandeExpert) line.getRootEntity();
          ...
          if(command.startsWith("UPDATE_")){
              DemandeExpert transientEntity = (DemandeExpert) line.getRootEntity();
