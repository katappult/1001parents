...
@Component("RessourcesBatchWriter")
@Scope("step")
public class RessourcesBatchWriter implements ItemWriter<AbstractLineBean> {

    private static final Logger LOGGER = LoggerFactory.getLogger(RessourcesBatchWriter.class);

    @Autowired
    private KatappultCoreServicesHelper services;

    @Autowired
    private IRessourcesService service;
    ...
    public void write(final List<? extends AbstractLineBean> items) throws Exception {
        ...
        while(entityIterator.hasNext()) {
          ...
          if(command.startsWith("ADD_")){
              ...
              Ressources entity = (Ressources) line.getRootEntity();
          ...
          if(command.startsWith("UPDATE_")){
              Ressources transientEntity = (Ressources) line.getRootEntity();
