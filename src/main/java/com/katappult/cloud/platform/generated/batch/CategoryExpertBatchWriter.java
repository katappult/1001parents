...
@Component("CategoryExpertBatchWriter")
@Scope("step")
public class CategoryExpertBatchWriter implements ItemWriter<AbstractLineBean> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CategoryExpertBatchWriter.class);

    @Autowired
    private KatappultCoreServicesHelper services;

    @Autowired
    private ICategoryExpertService service;
    ...
    public void write(final List<? extends AbstractLineBean> items) throws Exception {
        ...
        while(entityIterator.hasNext()) {
          ...
          if(command.startsWith("ADD_")){
              ...
              CategoryExpert entity = (CategoryExpert) line.getRootEntity();
          ...
          if(command.startsWith("UPDATE_")){
              CategoryExpert transientEntity = (CategoryExpert) line.getRootEntity();
