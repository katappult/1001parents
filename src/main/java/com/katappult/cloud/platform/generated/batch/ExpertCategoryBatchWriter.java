...
@Component("ExpertCategoryBatchWriter")
@Scope("step")
public class ExpertCategoryBatchWriter implements ItemWriter<AbstractLineBean> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExpertCategoryBatchWriter.class);

    @Autowired
    private KatappultCoreServicesHelper services;

    @Autowired
    private IExpertCategoryService service;
    ...
    public void write(final List<? extends AbstractLineBean> items) throws Exception {
        ...
        while(entityIterator.hasNext()) {
          ...
          if(command.startsWith("ADD_")){
              ...
              ExpertCategory entity = (ExpertCategory) line.getRootEntity();
          ...
          if(command.startsWith("UPDATE_")){
              ExpertCategory transientEntity = (ExpertCategory) line.getRootEntity();
