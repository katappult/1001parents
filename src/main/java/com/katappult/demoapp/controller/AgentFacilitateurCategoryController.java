...
@ManagedBean
@ViewScoped
public class AgentFacilitateurCategoryController extends BaseCRUDController implements Serializable {

  private static final Logger LOGGER = LoggerFactory.getLogger(AgentFacilitateurCategoryController.class);

  private String entityName = "AgentFacilitateurCategory";
  private String entityNameLower = "agentFacilitateurCategory";
  ...
  private AgentFacilitateurCategory selectedElementForDetails;
  private AgentFacilitateurCategory newElement = new AgentFacilitateurCategory();

  @Inject private IAgentFacilitateurCategoryService entityService;
  ...
  public void _doDeleteElement(Long oid) {
    ...
    IPersistable elementToDelete = services.persistableService().findById(oid, AgentFacilitateurCategory.class);
    ...
    try {
      entityService.delete((AgentFacilitateurCategory) elementToDelete, getWorkingContainer());
    } catch (Exception ex) {
  ...
  public String _doCreateNewElementAction() {
    ...
    try {
      ...
      AgentFacilitateurCategory createdElement = entityService.create(uiAttributes, getWorkingContainer());

      newElement = new AgentFacilitateurCategory();
    ...
    if (shouldReLoadDatas || refresh) {
      if (shouldReLoadDatas) {
        selectedElementForDetails =
            (AgentFacilitateurCategory) services.persistableService().findById(Long.valueOf(id), AgentFacilitateurCategory.class);
      }
      ...
      if (refresh) {
        selectedElementForDetails =
            (AgentFacilitateurCategory)
                services
                    .persistableService()
                    .findByIdFetchType(selectedElementForDetails.getOid(), AgentFacilitateurCategory.class);
      }
    }
  }
  ...
  public AgentFacilitateurCategory getSelectedElementForDetails() {
    loadDetails(false);
    return selectedElementForDetails;
  }

  public void setSelectedElementForDetails(AgentFacilitateurCategory selectedElementForDetails) {
    this.selectedElementForDetails = selectedElementForDetails;
  }

  public AgentFacilitateurCategory getNewElement() {
    return newElement;
  }

  public void setNewElement(AgentFacilitateurCategory newElement) {
