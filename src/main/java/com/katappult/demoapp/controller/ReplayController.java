...
@ManagedBean
@ViewScoped
public class ReplayController extends BaseCRUDController<Replay> implements Serializable {
  ...
  @Inject
  private IReplayService entityService;
  ...
  public void _doUpdateElement() {
    ...
    try {
      ...
      Replay element = entityService.update(uiAttributes, getWorkingContainer());
  ...
  private void loadDetails(boolean refresh) {
    ...
    if (shouldReLoadDatas || refresh) {
      ...
      if (shouldReLoadDatas) {
        Replay element =
            (Replay) services.persistableService().findById(targetId, Replay.class);
        selectedElementForDetails.setElement(element);
      }
      ...
      if (refresh && previousId != null) {
        Replay element =
            (Replay)
                services
                    .persistableService()
                    .findByIdFetchType(previousId, Replay.class);
        selectedElementForDetails.setElement(element);
      }
  ...
  @Override
  public Class<Replay> getTargetEntityClass() {
    return Replay.class;
  }
