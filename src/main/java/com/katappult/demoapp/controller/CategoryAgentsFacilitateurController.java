...
import com.katappult.cloud.platform.generated.model.CategoryAgentsFacilitateur;
...
@ManagedBean
@ViewScoped
public class CategoryAgentsFacilitateurController extends BaseCRUDController implements Serializable {

  private static final Logger LOGGER = LoggerFactory.getLogger(CategoryAgentsFacilitateurController.class);

  private String entityName = "CategoryAgentsFacilitateur";
  private String entityNameLower = "categoryAgentsFacilitateur";
  ...
  private CategoryAgentsFacilitateur selectedElementForDetails;
  private CategoryAgentsFacilitateur newElement = new CategoryAgentsFacilitateur();

  @Inject private ICategoryAgentsFacilitateurService entityService;
  ...
  public void _doDeleteElement(Long oid) {
    ...
    IPersistable elementToDelete = services.persistableService().findById(oid, CategoryAgentsFacilitateur.class);
    ...
    try {
      entityService.delete((CategoryAgentsFacilitateur) elementToDelete, getWorkingContainer());
    } catch (Exception ex) {
  ...
  public String _doCreateNewElementAction() {
    ...
    try {
      ...
      CategoryAgentsFacilitateur createdElement = entityService.create(uiAttributes, getWorkingContainer());

      newElement = new CategoryAgentsFacilitateur();
    ...
    if (shouldReLoadDatas || refresh) {
      if (shouldReLoadDatas) {
        selectedElementForDetails =
            (CategoryAgentsFacilitateur) services.persistableService().findById(Long.valueOf(id), CategoryAgentsFacilitateur.class);
      }

      if (refresh) {
        selectedElementForDetails =
            (CategoryAgentsFacilitateur)
                services
                    .persistableService()
                    .findByIdFetchType(selectedElementForDetails.getOid(), CategoryAgentsFacilitateur.class);
      }
    }
  }
  ...
  public CategoryAgentsFacilitateur getSelectedElementForDetails() {
    loadDetails(false);
    return selectedElementForDetails;
  }

  public void setSelectedElementForDetails(CategoryAgentsFacilitateur selectedElementForDetails) {
    this.selectedElementForDetails = selectedElementForDetails;
  }

  public CategoryAgentsFacilitateur getNewElement() {
    return newElement;
  }

  public void setNewElement(CategoryAgentsFacilitateur newElement) {
