...
@ManagedBean
@ViewScoped
public class CreditController extends BaseCRUDController<Credit> implements Serializable {
  ...
  @Inject
  private ICreditService entityService;
  ...
  public void _doUpdateElement() {
    ...
    try {
      ...
      Credit element = entityService.update(uiAttributes, getWorkingContainer());
  ...
  private void loadDetails(boolean refresh) {
    ...
    if (shouldReLoadDatas || refresh) {
      ...
      if (shouldReLoadDatas) {
        Credit element =
            (Credit) services.persistableService().findById(targetId, Credit.class);
        selectedElementForDetails.setElement(element);
      }
      ...
      if (refresh && previousId != null) {
        Credit element =
            (Credit)
                services
                    .persistableService()
                    .findByIdFetchType(previousId, Credit.class);
        selectedElementForDetails.setElement(element);
      }
  ...
  @Override
  public Class<Credit> getTargetEntityClass() {
    return Credit.class;
  }
