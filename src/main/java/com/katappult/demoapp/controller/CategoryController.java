...
@ManagedBean
@ViewScoped
public class CategoryController extends BaseCRUDController<Category> implements Serializable {

  private static final Logger LOGGER = LoggerFactory.getLogger(CategoryController.class);

  private String entityName = "Category";
  private String entityNameLower = "category";
  ...
  private Category newElement = new Category();

  @Inject
  private ICategoryService entityService;
  ...
  public void _doDeleteElement(Long oid) {
    ...
    IPersistable elementToDelete = services.persistableService().findById(oid, Category.class);
    ...
    try {
      entityService.delete((Category) elementToDelete, getWorkingContainer());
    } catch (Exception ex) {
  ...
  public String _doCreateNewElementAction() {
    ...
    try {
      ...
      Category createdElement = entityService.create(uiAttributes, getWorkingContainer());

      newElement = new Category();
  ...
  public void _doUpdateElement() {
    ...
    try {
      ...
      Category element = entityService.update(uiAttributes, getWorkingContainer());
  ...
  private void loadDetails(boolean refresh) {
    ...
    if (shouldReLoadDatas || refresh) {
      ...
      if (shouldReLoadDatas) {
        Category element =
            (Category) services.persistableService().findById(targetId, Category.class);
        selectedElementForDetails.setElement(element);
      }
      ...
      if (refresh && previousId != null) {
        Category element =
            (Category)
                services
                    .persistableService()
                    .findByIdFetchType(previousId, Category.class);
        selectedElementForDetails.setElement(element);
      }
  ...
  @Override
  public Class<Category> getTargetEntityClass() {
  ...
  public Category getNewElement() {
    return newElement;
  }

  public void setNewElement(Category newElement) {
